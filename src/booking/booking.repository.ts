...
export class BookingRepository extends Repository<Booking> {
  ...
  async createBooking(
    ...
    query
      ...
      .where(
        '(room.id = :id)AND (bookings.statusBooking != :Cancelled) AND (bookings.statusCheckin != :Checkout) AND ((bookings.checkin <= :checkin AND checkout >= :checkout) OR (checkin < :checkin  AND checkout >= :checkout) OR (checkin >= :checkin AND checkout <= :checkout) OR (checkin >= :checkin AND checkin <= :checkout) OR ( checkin  <= :checkin AND (checkout >= :checkin AND checkout <= :checkout)))',
        {
          id: roomid,
          Cancelled: 'CANCELLED',
          Checkout: 'CHECKEDOUT',
  ...
  async checkInOutUser(
    ...
    query
      ...
      .where('bookings.book_id = :id and facility.ownerID = :userid', {
        id: id,
        userid: user.id,
      });
    const query1 = await query.getOne();
