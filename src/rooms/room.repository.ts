...
export class RoomRepository extends Repository<Room> {
  ...
  async getRooms(
    ...
    if (type.localeCompare('hotel') === 0) {
      const query = this.createQueryBuilder('room')
        ...
        .select(['facility.id', 'room.id', 'facility.status'])
      ...
      if (minimum && maximum) {
        query.andWhere('room.cost >= :minimum AND room.cost <= :maximum', {
          minimum,
          maximum,
        });
      }
      ...
      if (checkin && checkout) {
        const query1 = this.createQueryBuilder()
        ...
        query1.where(
          '(bookings.statusBooking != :Cancelled) AND (bookings.statusCheckin != :Checkout) AND ((bookings.checkin <= :checkin AND checkout >= :checkout) OR (checkin < :checkin  AND checkout >= :checkout) OR (checkin >= :checkin AND checkout <= :checkout) OR (checkin >= :checkin AND checkin <= :checkout) OR ( checkin  <= :checkin AND (checkout >= :checkin AND checkout <= :checkout)))',
          { Cancelled: 'CANCELLED', Checkout: 'CHECKEDOUT', checkin, checkout },
        );
      ...
      if (search) {
        query.andWhere(
          '(room.title LIKE :search OR room.description LIKE :search OR room.status LIKE :search)',
          { search: `%${search}%` },
        );
      }
      ...
      if (checkin && checkout) {
        ...
        if (bookId.length > 0) {
          if (hotelid) {
            query.where(
              'room.id NOT IN (:...ids) AND room.facility.id = :hotelId',
              { ids: notAvailable, hotelId: hotelid },
            );
          }
          if (roomid) {
            query.where('room.id NOT IN (:...ids) AND room.id = :roomId', {
              ids: notAvailable,
              roomId: roomid,
            });
          }
  ...
  async getPrice(): Promise<any> {
    ...
    const query = this.createQueryBuilder('room');
    details.push(
      await query.select('DISTINCT(room.category)', 'category').getRawMany(),
    );
    details.push(
      await query
        .select('MIN(room.cost)', 'minimum')
  ...
  async createRoom(
    ...
    for (let i = 0; i < noOfRooms; i++) {
      ...
      room.title = title;
      if (features === 'null') room.features = null;
      else room.features = features;
